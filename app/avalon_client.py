...
def parse_stats(stats: dict[str, Any]) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        "workmode": None,
        "workmode_name": None,
        "state": None,           # 0=init, 1=working, 2=idle/standby
        "system_status": None,
        "temp_chassis": None,
        "temp_max": None,
        "temp_avg": None,
    ...
    # If STATE is "idle" (2), override workmode_name to "standby" so the UI
    # reflects the real running state, regardless of the configured workmode.
    if out["state"] == 2:
        out["workmode_name"] = "standby"
    elif out["workmode"] is not None:
...
def _scan_mm_payload(payload: str, out: dict[str, Any]) -> None:
    ...
    while i < n:
        ...
        if key == "WORKMODE":
            ...
            # 0=init/boot, 1=working, 2=idle (soft-off/standby)
            out["state"] = _safe_int(value)
        elif key == "SYSTEMSTATU":
            out["system_status"] = value.strip()
        elif key == "ITemp":
